...
namespace OptimalRoute.Application.Services
{
    public class RouteService : IRouteService
    {
        public (List<string> route, int totalTime) CalculateShortestRoute(List<string> cities, List<(string from, string to, int time)> roads, string origin, string destination)
        {
            try
            {
                if (string.IsNullOrEmpty(origin) || string.IsNullOrEmpty(destination) || !cities.Contains(origin) || !cities.Contains(destination))
                {
                    throw new ArgumentNullException($"The specified origin '{origin}' or destination '{destination}' does not exist.");
                }
                ...
                foreach (var road in roads)
                {
                    ...
                    if (road.time <= 0)
                    {
                        throw new ArgumentException("All roads must have a positive weight (time > 0).");
                    }
                }
                ...
                if (!route.Contains(origin) || !route.Contains(destination))
                {
            ...
            {
                throw new Exception($"Error calculating the shortest route: {ex.Message}", ex.InnerException);
            }
        }
    }
}
